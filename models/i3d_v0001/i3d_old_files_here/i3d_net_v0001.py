...
if __name__ == '__main__':
    ...
    model.to(device)
    if is_load:
        ckpt_path = './models/i3d/checkpoints/i3d_rgb.pt'
    ...
    rgb_dev255 = rgb / 255.
    rgb_permute = torch.permute(rgb_dev255, [3,0,1,2])

    rgb_resize = resize.forward(rgb_permute)
    ...
    # slices = form_slices(video_info['duration'], stack_size=16, step_size=16)
    # input_features = torch.rand(1,3,16,224,224)
    output_list = []
    for startp in slice_start:
        ...
        # ten_crop_list = []
        # for one_crop in rgb_oneslice:
        #     one_crop = torch.unsqueeze(one_crop, dim=0)
        #     input_features = one_crop.to(device)
        #     with torch.no_grad():
        #         output = model.forward(input_features, features=True)
        #     ten_crop_list.append(output)
        
        # output_tencrop = torch.stack(ten_crop_list)
